\documentclass[ %	msc, %% Para dissertações de mestrado, OU
...
%		coorientador={Ciclano}, %% Para masculino
	...
	\chapter{Introdução}\label{intro}
        ...
        \begin{figure}[H]
            ...
            \label{glaucomadiff}
            \includegraphics[width=0.98\textwidth]{./img/glaucoma_diff.png}
            \small{Fonte:
        ...
        \begin{figure}[H]
            ...
            \label{octdiff}
            \includegraphics[width=0.98\textwidth]{./img/oct.jpg}
            \small{Fonte:
    ...
    doenças nas folhas da maçã \citep{Li2021-vk}. De acordo com os autores, o de dados é conjunto
   ...
   ~\ref{sec:batch} e também qual otimizador~\ref{sec:optimizer} será utilizado \citep{GoodBengCour16}. Normalmente, para
    ...
    \end{subfigure}
    \small{Fonte: Imagem extraída do conjunto de dados.}
    \end{figure}
   ...
   ~\ref{fig:fundus2b}, que possui um zoom mais próximo do
    ...
    somente a camada nova adicionada com o conjunto de dados que queremos utilizar.
   ...
   implementados dentro da biblitoeca Pytorch\footnote{Biblioteca Python de aprendizado de máquina}.
    ...
    \begin{figure}[H]
        ...
        \includegraphics[width=0.90\textwidth]{./img/cross_compare.png}
        \small{Fonte: Elaborado pelo autor}
    \end{figure}

    A tabela a seguir~\ref{tab:crosslrd} sumariza as melhorias da aplicação da técnia LRD. A
    tabela mostra que houve melhorias nas métricas AUC, SP e SN.  O modelo que mais se beneficiou
    foi o modelo RegNet16x quanto a AUC, no entanto possui uma variabilidade alta nos resultados.
    Outra fator importante foi a perda de desempenho do modelo ResNet. O motivo por trás disto foi
    que a aplicação da técnica \textit{Early Stopping} fez com que o modelo parasse seu treinamento
    antes da hora após 10 épocas sem melhorar o desempenho em AUC. O desvio padrão maior está no
    grupo de médias das métricas, os valores máximos de desempenho atingido pelos modelos trouxeram
    menos variabilidade. Para os resultados completo, uma tabela contendo os hiperpâmetros
    utilizados e os resultados detalhados estará presente no apêndice dessa dissertação.

    \input{./tables/cross.tex}


    \input{./tables/meminf.tex}

    Quanto ao uso de memória e tempo de inferência, é possível observar na tabela~\ref{tab:meminf}
    que o modelos RegNets possuem  tempo de inferência próximo dos modelos mais simples, como ShuffleNet e MobileNet, e
    menor do que os modelos mais complexo, como por exemplo o Inception. Isto proporciona flexibilidade
    para que o modelo RegNet possa ser utilizado em diferente contextos que é necessário uma
    resposta rápida, como por exemplo processamento de quadros de um video ou em algum sistema nuvem que recebe
    milhares de requisições ao mesmo tempo. Quanto ao uso de memória, os modelos RegNets apresentam
    versões com diferentes uso de recursos computacionais. Variando de 1600 até
    4182 megabytes, o modelo consegue apresentar desempenho melhor ou próximo do que os modelos que
    utilizam mais memória, e também aqueles que são mais eficientes computacionalmente.

    \subsection{\textit{RandAugment}}

    Dado os resultados da seção anterior, foram selecionados 3 modelos para aplicar \textit{data
    augmentation}. Um dos hiperparâmetros do \textit{RandAugument} (RandOP na seguinte tabela)
    é o nome de operações sequenciais em uma foto. Por esse motivo, foram testados diferentes valores para este hiperparâmetro: 1, 2,
    3 e 4. Nesta etapa também foi utilizada validação cruzada para avaliar o modelo. A
    tabela~\ref{tab:randaug} a seguir mostra os resultados dos experimentos e os respectivos
    hiperparâmetros associado a este experimento:

    \input{./tables/randaug.tex}

    O \textit{RandAugment} trouxe uma variância menor nas métricas, principalmente na AUC. Dado isso,
    é possível verificar que o \textit{DataAugmentation} é uma ferramenta para lidar
    com o problema de sobreajsute~\ref{sec:overfitting} dos modelos quando o número de dados
    disponíveis é pequeno e uma heterogeneidade irá trazer benefícios quanto a generalização do
    modelo.
    ...
    \subsection{Explicabilidade Visual}



    A fígura abaixo~\ref{fig:shapimageeye1} compara duas arquiteturas de modelos. A da esquerda
    utlizando somente uma imagem e a da direita utilizando duas imagens. Na primeira foto, existe
    uma concentração de pontos dentro dos círculos e também alguns pontos na linha inferior da
    imagem. Isto se da ao fato que a arquitetura utilizando somente uma entrada com um modelo visual
    deve lidar com os dois tipos de imagem citadas~\ref{fig:fundus2}. No entanto, o modelo da
    direita reserva um modelo visual pra cada umas das imagens. No entanto, na segunda imagem os
    dois modelos deram exatamente a mesma explicabilidade para a imagem do fundo do olho. Desta
    forma, podemos perceber que o modelo utilizando uma imagem consegue se adaptar na utilização de
    duas imagens de fundo de olho.

    \begin{figure}[h]
        \centering
        \caption{Imagem com explicabilidade do modelo. Sendo o da esquerda o modelo utilizando
        somente uma imagem e o da direita utilizando duas imagens. Foto distante.}
        \label{fig:shapimageeye1}
        \includegraphics[width=0.98\textwidth]{./img/shap_eye.png}
        \small{Fonte: Elaborado pelo autor.}
    \end{figure}
    ...
    \begin{figure}[H]
        \centering
        \caption{Imagem com explicabilidade do modelo. Sendo o da esquerda o modelo utilizando
        somente uma imagem e o da direita utilizando duas imagens. Foto próxima.}
        \label{fig:shapimageeye2}
        \includegraphics[width=0.98\textwidth]{./img/shap_eye2.png}
        \small{Fonte: Elaborado pelo autor.}
    \end{figure}

    A segunda fígura~\ref{fig:shapimageeye2} de explicabilidade de imagem traz a explicabilidade
    para imagens mais próximas do fundo do olho, sem a margem preta presente no primeiro
    tipo~\ref{fig:shapimageeye1}. O primeiro modelo adaptou a explicabilidade para extrair
    características desse tipo de imagem, formando círculos e uma pequena concentração arrendondada
    no meio. Da mesma forma como na fígura anterior~\ref{fig:shapimageeye1}, o primeiro modelo possui
    uma concentração na linha inferior também. Estas concentrações inferior se da ao fato que o
    disco e copo ótico podem estar em qualquer posição da imagem, e para isto, o modelo precisa
    analisar todos os cantos possíveis. Um ponto de atenção, é que o modelo utilizando duas imagens
    como entrada, o da direita, praticamente não extraiu nenhum benefício da segunda foto. Isto
    corrobora com os resultados obtidos nesta tabela~\ref{tab:aucdualoct}, onde o modelo de duas imagens nada acrescenta
    para o desempenho da tarefa.

    E por útlimo, podemos perceber que a explicabilidade e concentrada nas mesmas regiões tanto para
    identificar a presença do glaucoma quanto a sua ausência. Isto é explicado pela mistura de
    pontos azuis (diagnóstico negativo) e pontos vermelhos (diagnóstico positivo) se misturando na
    área da imagem em geral.


    \subsection{Importância das Características}

    Nesta seção iremos avalair a importância de cada um das características utilizada pelo modelo,
    inclusive aquelas que foram criadas atráves de engenharia de características. As análises a
    seguir mostra quais foram os dados mais importantes para modelo quanto ao seu diagnósticos. Aleḿ
    disso, será possível verificar como cada um das características influência na decisão do valor a
    depender do seu valor.

    \begin{figure}[H]
        \centering
        \caption{Imprtância das características calculada utilizando o melhor modelo usando OCT (AUC
        = 0.99).}
        \label{fig:shapimp}
        \begin{subfigure}[ht]{0.48\textwidth}
            \centering
            \caption{Importância das características nos dados de treino do conjunto AC.}
            \label{fig:shapimp1}
            \includegraphics[width=0.98\textwidth]{./img/shap_train_importance.png}
        \end{subfigure}
        \hfill
        \begin{subfigure}[ht]{0.48\textwidth}
            \centering
            \caption{Importância das características nos dados de teste AD.}
            \label{fig:shapimp2}
            \includegraphics[width=0.98\textwidth]{./img/shap_val_importance.png}
        \end{subfigure}
        \small{Fonte: Elaborado pelo autor.}
    \end{figure}

    A fígura~\ref{fig:shapimp} mostra, de cima para baixo, as características mais importantes para
    os modelos. O gráfico da esquerda é do conjunto AC e o da direita do AD. As 3 primeiras
    características, valor médio global (G), temporal superior (TS) e temporal inferior (TI) estão
    nas mesmas posições para os dois conjuntos de dados. Isto mostra, que os dois conjunto de dados
    possuem distribuição quanto a sua importância. Relativo as outras características, sua
    imprtância mudam entre o conjunto de dados, no entanto, nenhuma mudança que seja significativa.
    As fíguras~\ref{fig:shapimp1} e~\ref{fig:shapimp2} demostra que quando a mudança quanto a
    importância a variação costuma ser de 1,2 ou 3 posições.

    Ainda analisando a mesma fígura~\ref{fig:shapimp} que valores grande para TS/G/TI, ou seja, mais
    presença de fibras na parte temporal e na média total esta associado a ausência do glaucoma.
    Outro ponto importante é que os dados afirma que quanto mais avançado a idade maior as chances
    de diagnosticar o glaucoma. Quanto ao gênero o modelo não trouxe nenhum tipo de relação
    importante. Para as características criadas sintéticamente, T/TS mostra que quando a razão
    aumenta, ou seja, quando o total de fibras na região temporal T diminuem, isto ocorre de forma
    distribuída entre TS e TI, já que se TS diminuísse mais rápido que TI é provável que a razão
    aumentaria diminuindo as chances do glaucoma. Para N/TI pode ser o oposto, ou seja, significa
    que as fibras se degradam mais rápido na área temporal comparado a Nasal (N).

    \begin{figure}[H]
        \centering
        \caption{Mapa de calor importância das características para os backbones presentes na etapa de
        validação cruzada. As características estão ordenada de acordo com sua importância geral.}
        \label{fig:heatmap_imp}
        \includegraphics[width=0.98\textwidth]{./img/heatmap_imp.png}
        \small{Fonte: Elaborado pelo autor.}
    \end{figure}

    O mapa de calor~\ref{fig:heatmap_imp} mostra que TS assume um papel importante em quase todos os
    modelos. Ou seja, podemos dizer que esta característica é de fato importante para diagnóstico do
    glaucoma pelo exame OCT e deve ser analisada com cuidado pelos profissionais. Em suma, toda as
    características relacionada a área temporal, com exceão das criadas sinteticamente, ficaram
    acima das características nasais, dando ainda mais relevância para esta região. A área TI
    assumiu um papel importante quando se utiliza o \textit{backbone} do modelo Mobile. Outra
    análise importante é que este mesmo \textit{backbone} trouxe importância para as caraterísticas
    que envolve gênero e idade, que foi de certa forma irrelevante para os outros modelos. Algumas
    outras características parece ser irrelevantes para qualquer modelo, como por
    exemplo: TS/NI, T/NI, TS/NS, NS/NI, T/N, NS/N  e TS/TI. Para características não-sintéticas e
    regionais, a menos relevante para a modelagem foi a área nasal geral (N).


    \subsection{Análise de casos individuais}

    Neste seção iremos avaliar casos individuais de decisão do modelo. Na primeira parte sera
    analisado dois casos com glaucoma onde o modelo tomou a decisão certa em um e errou na outra.
    Para isto, iremos o gráfico de cascata (\textit{waterfall}) do SHAP, onde a temos cada um dos
    atributos do paciente e como cada um deste contribui em magnitude para a decisão do modelo de
    acordo com os valores do atributo.

    \begin{figure}[H]
        \centering
        \caption{Gráfico de cascata para casos com presença de glaucoma.}
        \label{fig:wpos}
        \begin{subfigure}[ht]{0.48\textwidth}
            \centering
            \caption{Decisão errada do modelo.}
            \label{fig:wpos1}
            \includegraphics[width=0.98\textwidth]{./img/case62.png}
        \end{subfigure}
        \hfill
        \begin{subfigure}[ht]{0.48\textwidth}
            \centering
            \caption{Decisão correta do modelo.}
            \label{fig:wpos2}
            \includegraphics[width=0.98\textwidth]{./img/case105.png}
        \end{subfigure}
        \small{Fonte: Elaborado pelo autor.}
    \end{figure}


    A fígura~\ref{fig:wpos} possui dois gráficos de cascata, sendo o da direita o modelo tomou a
    decisão correta e o da esquerda errada. O valor abaixo na fígura mostra o valor esperado $E[f(x)]$, ou
    seja, a média de decisão do modelo. Para valores acima dessa média o modelo toma o diagnóstico
    como positivo, caso contrário negativo. A saída do modelo é visto na parte superior do gráfico
    $f(x)$. A média global G foi a característica que mais contribuí para decisão do modelo. Pela
    distribuição da fígura~\ref{fig:dist_glaucoma} é possível verificarmos que valores de G entre 40
    e 70 é composto grupo de pessoas que estão diagnósticado com o Glaucoma. O mesmo vale para TS e
    para T, apesar da última estar no limiar entre a população com Glaucoma e sem Glaucoma. A idade
    de 75 anos foi um fator também que contribui para o modelo diagnosticar como positivo. No
    gráfico de cascata da esquerda, a característica mais importante foi NS com um valor de 155.
    Este valor, dado a fígura~\ref{fig:dist_glaucoma}, está na cauda da população com Glaucoma e
    uma concentração maior de casos com a ausência da doença, o que pode ter confundido o modelo.


    \begin{figure}[H]
        \centering
        \caption{Gráfico de cascata para casos com ausência de glaucoma.}
        \label{fig:wneg}
        \begin{subfigure}[ht]{0.48\textwidth}
            \centering
            \caption{Decisão errada do modelo.}
            \label{fig:wneg1}
            \includegraphics[width=0.98\textwidth]{./img/case127.png}
        \end{subfigure}
        \hfill
        \begin{subfigure}[ht]{0.48\textwidth}
            \centering
            \caption{Decisão correta do modelo.}
            \label{fig:wneg2}
            \includegraphics[width=0.98\textwidth]{./img/case7.png}
        \end{subfigure}
        \small{Fonte: Elaborado pelo autor.}
    \end{figure}

    Na fígura~\ref{fig:wneg1} o valor de TS e razão entre T/TS foram o que causaram distorção na
    saída do modelo. Com a medição de TS igual a 12, a razão ficou 6 vezes maior entre T e TS. O
    valor de TS pode ser um possível outlier, dado a fígura~\ref{fig:dist_glaucoma}, um problema de medição
    ou apenas um erro esporádica na anotação dos dados. No entanto, de qualquer forma, o modelo foi
    impactado por este valor e como consequência um diagnóstico positivo errado. Quanto ao
    diagnóstico da imagem da direita~\ref{fig:wneg2}, é possível verificarmos que TI tem mais que o
    dobro de fibra de N e utilizando dessa informação o modelo foi capaz de verificar a ausência do
    glaucoma.
    ...
    O \textit{Rand Augmentation} mostrou benefícios quanto a variabilidade dos resultados, sendo uma
    ferramenta essencial para evitar o sobreajuste.. Por outra lado, a
    técnica de \textit{Learning Rate Decay} apresentou benefícios para os modelos, aumentando seu
    desempenho quanto a métrica AUC e as outras métricas SP e SN, porém, aumentando também a
    variabilidade do modelo quanto a estas métricas.

    O modelo trouxe uma explicabilidade para corroborar com os resultados. A arquitetura que utiliza
    somente uma imagem como entrada (SI) consegue extrar características de diferentes fotos de
    fundo de olho, sem a necessidade de se ter um segundo modelo para processamento de uma segunda
    imagem. As características G, T e TS foram as mais importantes para os modelos utilizando dados
    clínicos tabulares. Alem disso, utilizando casos individuais, conseguimos observar como cada uma
    das características pode desempenhar um papel fundamental na decisão do modelo, seja ela correta
    ou não.
    ...
    \begin{apendices}
        ...
        \chapter{Validação Cruzada Resultados}
        \label{ap:crosscomplete}
        A tabela a seguir mostra os resultados completos obtidos pela utilização do \textit{Learning
        Decay} (LRD) comparado aos experimentos sem sua utlização (C).
        \input{./tables/cross\_complete.tex}
    \end{apendices}
